...
export default function WorkerDashboard() {
  ...
  const executeSearch = useCallback(async (query) => {
    ...
    try {
      const token = localStorage.getItem('wms_auth_token');
      const headers = token ? { 'Authorization': `Bearer ${token}` } : {};
      const res = await fetch(`/api/orders?orderNo=${encodeURIComponent(term)}`, { headers });
      const data = await res.json();
  ...
  return (
    <div>
      ...
      <nav className="glass-nav">
        <div className="brand-badge">
          ...
          <div>
            ...
            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600 }}>
              Floor Operator Station &bull; Barcode Dispatch
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
          <button
            ...
            style={{ fontSize: '0.8rem', padding: '0.35rem 0.8rem', borderRadius: 'var(--radius-pill)' }}
          >
          ...
          <button
            ...
            style={{ padding: '0.4rem 0.8rem', fontSize: '0.82rem', borderRadius: 'var(--radius-pill)' }}
          >
            Sign Out
          </button>
        </div>
      ...
      <div className="document-container" style={{ margin: '1.5rem auto', maxWidth: '1280px', padding: '1.75rem' }}>
        {/* Mode Switcher */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '0.75rem' }}>
          ...
          {workerMode === 'single' && (
            <button
              ...
              style={{ padding: '0.55rem 1.35rem', fontSize: '0.9rem', borderRadius: 'var(--radius-pill)', boxShadow: '0 4px 14px rgba(16, 185, 129, 0.25)' }}
            >
