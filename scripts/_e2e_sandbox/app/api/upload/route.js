...
export async function POST(request) {
  ...
  try {
    ...
    // ── PHASE 2: Fetch existing orders in chunks to stay within SQLite limits ─
    const CHUNK_SIZE = 500;
    const existingOrders = [];
    for (let i = 0; i < allOrderNos.length; i += CHUNK_SIZE) {
      const chunk = allOrderNos.slice(i, i + CHUNK_SIZE);
      const ordersChunk = await prisma.order.findMany({
        where: { orderNo: { in: chunk } },
        select: {
      ...
      existingOrders.push(...ordersChunk);
    }
    ...
    // ── PHASE 4: Batch create + batch update in safe chunks ───────────────
    let addedCount = 0, updatedCount = 0;

    // Batch create in safe chunks of 50 (SQLite variable limit safety, no skipDuplicates)
    const CREATE_CHUNK = 50;
    for (let i = 0; i < toCreate.length; i += CREATE_CHUNK) {
      const chunk = toCreate.slice(i, i + CREATE_CHUNK);
      const created = await prisma.order.createMany({ data: chunk });
      addedCount += created.count;
    }

    // Batch update in chunks of 100 inside a transaction to avoid SQLite lock timeouts
    ...
    // Create import events in bulk for new orders (chunked)
    if (addedCount > 0) {
      const newOrderNos = toCreate.map(r => r.orderNo);
      const newOrders = [];
      for (let i = 0; i < newOrderNos.length; i += CHUNK_SIZE) {
        const chunk = newOrderNos.slice(i, i + CHUNK_SIZE);
        const ordersChunk = await prisma.order.findMany({
          where: { orderNo: { in: chunk } },
          select: { id: true, status: true }
        });
        newOrders.push(...ordersChunk);
      }

      if (newOrders.length > 0) {
        const eventData = newOrders.map(o => ({
          orderId: o.id,
          status: o.status,
          actorName: user.name,
          actorRole: user.role,
        ...
        for (let i = 0; i < eventData.length; i += CREATE_CHUNK) {
          const chunk = eventData.slice(i, i + CREATE_CHUNK);
          await prisma.orderEvent.createMany({ data: chunk });
        }
      }
    }
